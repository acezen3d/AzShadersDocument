Shader "Az/StandardSkin"
{
    Properties
    {
        ...
        [Enum(Normal, 0, Multiply, 1, MultiplyAndDouble, 2, Add, 3, AddMultiply, 4, Subtract, 5, ReverseSubtract, 6, Screen, 7, Max, 8, Min, 9, Overlay, 10, HardLight, 11)] _OverTex1BlendType ("OverTex1BlendType", Float) = 0
        _overtex2 ("overtex2", 2D) = "black" { }
        _overcolor2 ("overcolor2", Color) = (1, 1, 1, 1)
        [Enum(Normal, 0, Multiply, 1, MultiplyAndDouble, 2, Add, 3, AddMultiply, 4, Subtract, 5, ReverseSubtract, 6, Screen, 7, Max, 8, Min, 9, Overlay, 10, HardLight, 11)] _OverTex2BlendType ("OverTex2BlendType", Float) = 0
        _overtex3 ("overtex3", 2D) = "black" { }
        _overcolor3 ("overcolor3", Color) = (1, 1, 1, 1)
        [Enum(Normal, 0, Multiply, 1, MultiplyAndDouble, 2, Add, 3, AddMultiply, 4, Subtract, 5, ReverseSubtract, 6, Screen, 7, Max, 8, Min, 9, Overlay, 10, HardLight, 11)] _OverTex3BlendType ("OverTex3BlendType", Float) = 0
        _nip ("nip", Range(0, 1)) = 1
        _nipsize ("nipsize", Range(0, 1)) = 0.5
        _nip_specular ("nip_specular", Range(0, 1)) = 1
        _tex1mask ("tex1mask", Range(0, 1)) = 1
        // Extension / Over, Cover
        _OverTex ("OverTex", 2D) = "white" { }
        _OverColor ("OverColor", Color) = (1, 1, 1, 1)
        [Enum(Normal, 0, Multiply, 1, MultiplyAndDouble, 2, Add, 3, AddMultiply, 4, Subtract, 5, ReverseSubtract, 6, Screen, 7, Max, 8, Min, 9, Overlay, 10, HardLight, 11)] _OverTexBlendType ("OverTexBlendType", Float) = 1
        _CoverTex ("CoverTex", 2D) = "white" { }
        _CoverColor ("CoverColor", Color) = (1, 1, 1, 1)
        [Enum(Normal, 0, Multiply, 1, MultiplyAndDouble, 2, Add, 3, AddMultiply, 4, Subtract, 5, ReverseSubtract, 6, Screen, 7, Max, 8, Min, 9, Overlay, 10, HardLight, 11)] _CoverTexBlendType ("CoverTexBlendType", Float) = 1
        // Extension / Extra Normal Maps
        [Normal] _OverTex1NormalMap ("OverTex1NormalMap", 2D) = "bump" { }
        _OverTex1NormalMapScale ("OverTex1NormalMapScale", Range(0, 1)) = 1
        [Normal] _OverTex2NormalMap ("OverTex2NormalMap", 2D) = "bump" { }
        _OverTex2NormalMapScale ("OverTex2NormalMapScale", Range(0, 1)) = 1
        [Toggle] _UseBlueAsMaskForNormalMaps ("UseBlueAsMaskForNormalMaps", Float) = 0
        // Koikatsu / Liquid
    ...
    SubShader
    {
        Tags { "RenderType" = "TransparentCutout" "PerformanceChecks" = "False" "Queue" = "AlphaTest-100" }
        LOD 300
        Pass
        {
            CGPROGRAM
