...
| EmissionMapPremultiplyAlpha | Boolean       | true          | Whether to premultiply the `rgb` channels of the emission maps (`EmissionMap` and `ColorBlendMap`) by their `alpha`, consistent with the previous handling of the emission maps' `alpha` channels.              |
...
### MaskLevels
The level controls for the `Mask` corresponding channels. The sign of each component indicates whether the mask is inverted, while its absolute value from 0 to 2 represents: 0: fully suppressed; 1: mask-defined; 2: fully applied. For example:
