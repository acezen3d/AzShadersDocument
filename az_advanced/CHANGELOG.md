...
- Redesign the alpha dithering:
  ...
  - Add `Alpha` and the alpha dithering to the `Az/AdvancedHair` and `Az/Advanced*Cutout` shaders.
...
- Add the tear and wear features (in the Extra PBR category) to the cloth and item shaders.
...
- Add the stocking feature to the cloth shaders.
...
- Add the alpha dithering feature to `Az/AdvancedClothCutout`, `Az/AdvancedItemCutout` and `Az/AdvancedLiteCutout`.
...
- Enable the specular setup support on the cloth and item shaders.
  - Add the `SPECULAR_SETUP` keyword and the `SpecColor`, `DetailSpecularBlendType` properties.
